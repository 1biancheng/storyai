...
import { uploadAndExtractFile, splitParagraphs, Paragraph } from '../sample_data/services/paragraphService';
import { ingestStoryText } from '../sample_data/services/storyService';
...
const ParagraphProcessingTool: React.FC = () => {
  ...
  return (
    <div className="p-6 bg-white dark:bg-[#1E1E1E] h-full rounded-xl shadow-md border border-gray-200 dark:border-[rgba(255,255,255,0.05)]">
      <div className="flex items-center gap-3 mb-6">
        <Scissors size={24} className="text-gray-500 dark:text-gray-400" />
        <div>
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">段落处理工具</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400">文件上传 → 编码检测 → 段落拆分 → 入库管理</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-[calc(100%-60px)]">
        ...
        <div className="md:col-span-1 space-y-4">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 h-10 px-4 bg-gray-200 hover:bg-gray-300 dark:bg-white/10 dark:hover:bg-white/20 text-gray-800 dark:text-white font-semibold rounded-lg transition-colors"
                >
                  <Upload size={16} />
                  选择文件
                </button>
                ...
                {selectedFile && (
                  <button
                    ...
                    className="flex items-center justify-center gap-2 h-10 px-4 bg-blue-600 hover:bg-blue-500 text-white font-semibold rounded-lg transition-colors disabled:opacity-50"
                  >
                    {isUploading ? <Loader size={16} className="animate-spin" /> : <FileText size={16} />}
                    {isUploading ? '上传中...' : '上传'}
                  </button>
                )}
              </div>
              ...
              {/* 编码选择 */}
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">编码覆盖</label>
                <select
              ...
              {/* 文件元数据 */}
              {fileMetadata && (
                <div className="text-xs bg-gray-100 dark:bg-[#2C2C2C] p-2 rounded">
                  <p className="font-medium">检测信息:</p>
                  <p>编码: {fileMetadata.encoding} ({(fileMetadata.confidence * 100).toFixed(1)}%)</p>
                  <p>方法: {fileMetadata.method}</p>
                  <p>结构: {fileMetadata.structure}</p>
                </div>
              )}
            </div>
          </div>
          ...
          {fileText && (
            <div className="p-4 rounded-lg bg-gray-50 dark:bg-[#2C2C2C] border border-gray-200 dark:border-white/10">
              <h3 className="font-semibold text-gray-900 dark:text-white mb-3">拆分配置</h3>
              <div className="space-y-3">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">拆分方式</label>
                  <select
                    value={splitConfig.method}
                    onChange={(e) => setSplitConfig({...splitConfig, method: e.target.value as any})}
                    className="w-full h-8 px-2 text-sm border border-gray-300 dark:border-white/10 bg-white dark:bg-[#2C2C2C] rounded"
                  >
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">最小长度</label>
                    <input
                      type="number"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">清洗模式</label>
                  <select
                    value={splitConfig.clean_mode}
                    onChange={(e) => setSplitConfig({...splitConfig, clean_mode: e.target.value as any})}
                    className="w-full h-8 px-2 text-sm border border-gray-300 dark:border-white/10 bg-white dark:bg-[#2C2C2C] rounded"
                  >
        ...
        <div className="md:col-span-2">
          {paragraphs.length > 0 ? (
            ...
            <div className="h-full flex items-center justify-center bg-gray-50 dark:bg-[#2C2C2C] border border-dashed border-gray-200 dark:border-white/10 rounded-lg">
