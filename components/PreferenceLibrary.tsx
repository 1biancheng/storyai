...
const categoryTitles: Record<keyof PreferenceData, string> = {
    character_emotions: '人物情感',
    novel_genres: '小说类型',
    timeline_clues: '时代线索',
    narrative_perspectives: '叙述视角',
    main_plots: '主要情节',
    character_traits: '角色特点',
};

interface PreferenceLibraryProps {
  isOpen: boolean;
  onClose: () => void;
  project: ProjectData;
}
...
const PreferenceCategory: React.FC<{ title: string; items: PreferenceItem[]; hasData: boolean }> = ({ title, items, hasData }) => {
    // Sort by popularity if data exists
...
const PreferenceLibrary: React.FC<PreferenceLibraryProps> = ({ isOpen, onClose, project }) => {
  if (!isOpen) return null;

  const preferenceData = project.preferenceAnalysis || defaultPreferences;
  const hasAnalysisData = !!project.preferenceAnalysis;

  return (
    <div className="fixed inset-0 modal-overlay bg-black/60 dark:bg-black/70 z-50 flex items-center justify-center p-4" onClick={onClose} role="dialog" aria-modal="true" aria-labelledby="preference-library-title">
      <div className="bg-white dark:bg-[#1E1E1E] rounded-xl shadow-2xl w-full max-w-4xl h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="p-4 border-b border-gray-200 dark:border-white/10 flex-shrink-0 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <h2 id="preference-library-title" className="text-lg font-semibold text-gray-900 dark:text-[#E2E2E2]">
              偏好库: {project.projectName}
            </h2>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-white/10" aria-label="关闭偏好库">
            <X size={20} />
          </button>
        </div>
        <div className="flex-grow p-6 overflow-y-auto space-y-6">
            {!hasAnalysisData && (
                <div className="p-3 text-sm rounded-md bg-yellow-50 dark:bg-yellow-500/10 text-yellow-700 dark:text-yellow-300 border border-yellow-200 dark:border-yellow-500/20">
                    注意:当前显示的是默认关键词.运行包含"数据分析"节点的工作流后,将显示基于市场分析的实时热度.
                </div>
            )}
            {(Object.keys(preferenceData) as Array<keyof PreferenceData>).map(categoryKey => (
                 <PreferenceCategory 
                    key={categoryKey}
                    title={categoryTitles[categoryKey]} 
                    items={preferenceData[categoryKey]}
...
export default PreferenceLibrary;