...
import * as agentPrompts from '../sample_data/services/agentPrompts.ts';
...
const WorkflowNodeComponent: React.FC<{ node: WorkflowNode }> = ({ node }) => {
  ...
  const getIcon = useCallback(() => {
    switch (node.type) {
      ...
      case 'tool': 
        if (node.toolType === 'web_search') return <Search size={16} className="text-blue-500" />;
  ...
  }, [node.type, node.agentType, node.toolType]);
...
const LibraryPanel: React.FC = () => {
    ...
    const DraggableItem: React.FC<{ type: 'agent' | 'llm' | 'tool' | 'data', payload?: any, children: React.ReactNode }> = ({ type, payload, children }) => (
        <div draggable onDragStart={(e) => handleDragStart(e, { type, ...payload })} onDragEnd={handleDragEnd} className="p-2 border rounded-md bg-gray-50 dark:bg-[#2C2C2C] text-sm cursor-grab hover:bg-gray-100 dark:hover:bg-white/5 transition-colors">
            {children}
        </div>
    );
            ...
            <div className="p-4 space-y-4 overflow-y-auto">
                ...
                <div className="component-section">
                    ...
                    <div className="space-y-2">
                        ...
                        <DraggableItem type="tool" payload={{ toolType: 'web_search', name: '网络搜索' }}>🌐 网络搜索</DraggableItem>
                    </div>
                </div>
                <div className="component-section">
                ...
                <div className="space-y-4">
                    ...
                    {selectedNode.type === 'data' && (
                     ...
                     {selectedNode.type === 'tool' && (
                        <div className="space-y-3 border-t border-gray-200 dark:border-white/10 pt-4">
                            ...
                            {selectedNode.toolType === 'web_search' && (
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-400">搜索查询</label>
                                    <textarea value={selectedNode.searchQuery || ''} onChange={e => updateNodeProps(selectedNode.id, { searchQuery: e.target.value })} rows={3} className="mt-1 block w-full rounded-md border-gray-300 dark:border-white/10 bg-white dark:bg-[#2C2C2C] shadow-sm sm:text-sm" placeholder="输入搜索关键词..."></textarea>
                                    <div className="mt-2">
                                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-400">结果数量</label>
                                        <input type="number" value={selectedNode.numResults || 10} onChange={e => updateNodeProps(selectedNode.id, { numResults: parseInt(e.target.value) || 10 })} min="1" max="50" className="mt-1 block w-full rounded-md border-gray-300 dark:border-white/10 bg-white dark:bg-[#2C2C2C] shadow-sm sm:text-sm" />
                                    </div>
                                </div>
                            )}
                        </div>
                    ...
                    {selectedNode.type === 'agent' && selectedNode.promptCardId && (
                        <div className="border-t border-gray-200 dark:border-white/10 pt-4">
                            ...
                            {testResult && <textarea value={testResult} readOnly rows={6} className={`mt-2 block w-full rounded-md border-gray-300 dark:border-white/10 bg-gray-50 dark:bg-[#2C2C2C] sm:text-sm ${testStatus === 'error' ? 'text-red-700' : ''}`} />}
                        </div>
                    )}
                    ...
                    {nodeHistory.length > 0 && (
                        <div className="border-t border-gray-200 dark:border-white/10 pt-4">
                            ...
                            <div className="mt-2 border rounded-md bg-white dark:bg-[#2C2C2C] overflow-hidden max-h-48 overflow-y-auto">
                                {nodeHistory.map(log => (
...
const url = `${(import.meta as any).env?.VITE_BACKEND_URL || 'http://localhost:8000'}/api/v1/workflows/stream/${executionId}`;
