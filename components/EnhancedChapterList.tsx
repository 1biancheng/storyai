        ...
        {isBatchMode && (
          <div className="flex items-center justify-between p-2 bg-gray-50 rounded-md">
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                onClick={async () => {
                  if (!confirm('确定重新扫描文件系统并同步数据库？')) return;
                  try {
                    const projectId = chapters[0]?.projectId;
                    if (!projectId) return;
                    const res = await fetch(`/api/chapters/sync/${projectId}`, { method: 'POST' });
                    const data = await res.json();
                    alert(`同步完成：新增 ${data.inserted} 条，更新 ${data.updated} 条，缺失 ${data.missing} 条`);
                ...
                <div className="flex items-center p-2">
                  ...
                  >
                    ...
                    <div className="text-xs text-gray-500">
                      ...
                      {chapter.updatedAt && (
                        <span className="ml-2">
                          更新于 {new Date(chapter.updatedAt * 1000).toLocaleDateString()}
                        </span>
                      )}
                    </div>
                  </div>
