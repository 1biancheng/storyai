...
import { ingestStoryText, ingestBookStream, uploadBook, ingestFile } from '../sample_data/services/storyService.ts';
import EmotionCurveChart from './EmotionCurveChart.tsx';
import yaml from 'js-yaml';
import { CacheType, CacheManager } from '../sample_data/services/cacheManager.ts';
import { CustomType } from '../sample_data/services/agentFieldModels.ts';
  ...
  return (
    <div>
      ...
      <div className="space-y-2">
        {elements.map((el, i) => (
          <div key={i} className="bg-white dark:bg-[#2C2C2C] p-3 rounded-lg text-sm border border-gray-200 dark:border-white/10">
    ...
    <div className="space-y-6">
      <EmotionCurveChart points={result.emotionCurve} />
       <div>
          ...
          <p className="p-3 bg-white dark:bg-[#2C2C2C] rounded-lg text-sm border border-gray-200 dark:border-white/10">{result.overallSentiment}</p>
      </div>
        ...
        <div className="h-full flex flex-col">
            ...
            <div className="space-y-4 overflow-y-auto flex-grow pr-2">
                {visibleChunks.map((chunk, index) => {
                    ...
                    return (
                        <div key={index} className="bg-white dark:bg-[#2C2C2C] p-3 rounded-lg border border-gray-200 dark:border-white/10">
                            ...
                            <pre className="text-sm text-gray-700 dark:text-white/80 whitespace-pre-wrap font-sans bg-white dark:bg-[#2C2C2C] p-2 rounded max-h-48 overflow-auto">{chunk.content}</pre>
                        </div>
                    );
                })}
...
const DeconstructionTool: React.FC = () => {
    ...
    // 初始化时加载用户偏好
    React.useEffect(() => {
        const savedPreferences = CacheManager.get(CacheType.USER_PREFERENCES, 'deconstruction_tool');
        if (savedPreferences) {
    ...
    const saveUserPreferences = React.useCallback(() => {
        ...
        CacheManager.set(CacheType.USER_PREFERENCES, 'deconstruction_tool', preferences);
        ...
        properties: {
            emotionCurve: {
                ...
                items: {
                    type: CustomType.OBJECT,
                    properties: {
                        position: { type: CustomType.INTEGER, description: "The chapter number." },
                        score: { type: CustomType.NUMBER, description: "Emotional score from -1.0 (very negative) to 1.0 (very positive)." },
                        summary: { type: CustomType.STRING, description: "A one-sentence summary explaining the emotional context of the chapter." },
                        keywords: {
                            type: CustomType.ARRAY,
            ...
            characters: {
                ...
                items: {
                    type: CustomType.OBJECT,
                    properties: {
                        character: { type: CustomType.STRING, description: "Name of the character." },
                        description: { type: CustomType.STRING, description: "A brief description of the character's role, personality, and arc." },
                    },
            ...
            plotPoints: {
                ...
                items: {
                    type: CustomType.OBJECT,
                    properties: {
                        plotPoint: { type: CustomType.STRING, description: "The name or title of the plot point (e.g., 'Inciting Incident')." },
                        description: { type: CustomType.STRING, description: "A description of what happens at this plot point." },
                    },
            ...
            worldBuilding: {
                ...
                items: {
                    type: CustomType.OBJECT,
                    properties: {
                        element: { type: CustomType.STRING, description: "The name of the world-building element (e.g., 'Magic System', 'Political Factions')." },
                        description: { type: CustomType.STRING, description: "A description of this element." },
                    },
    ...
    const generateAnalysisPrompt = (text: string): string => {
        ...
        // 将schema转换为JSON字符串,以便包含在提示词中
        const schemaJson = JSON.stringify(bookAnalysisSchema, null, 2);
...
`;
    ...
    const handleAnalyze = async () => {
        ...
        try {
            const response = await agentService.runAgent(prompt);
            if (response.text) {
        ...
        <div className="p-6 bg-white dark:bg-[#1E1E1E] h-full overflow-y-auto text-gray-900 dark:text-[#E2E2E2] rounded-xl shadow-md border border-gray-200 dark:border-[rgba(255,255,255,0.05)]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-full">
                <div className="md:col-span-1 space-y-4 overflow-y-auto pr-2">
                    <div className="p-4 rounded-lg bg-gray-50 dark:bg-[#2C2C2C] border border-gray-200 dark:border-white/10">
                            ...
                            <div className="mt-2 p-3 bg-white dark:bg-[#121212] rounded-lg border border-gray-200 dark:border-white/10">
                              ...
                              {fileMetadata.paragraph_ids && fileMetadata.paragraph_ids.length > 0 && (
                                <div className="mt-2 text-xs">
                                  ...
                                  <div className="font-mono text-gray-800 dark:text-white bg-white dark:bg-[#2C2C2C] p-2 rounded max-h-20 overflow-auto text-[10px]">
                ...
                <div className="md:col-span-2 h-full">
                     {isAnalyzing ? (
                         <div className="h-full min-h-[300px] flex flex-col items-center justify-center bg-gray-50 dark:bg-[#2C2C2C] border border-dashed border-gray-200 dark:border-white/10 rounded-lg text-gray-400 dark:text-gray-500">
                        ...
                        <div className="h-full min-h-[300px] flex items-center justify-center bg-gray-50 dark:bg-[#2C2C2C] border border-dashed border-gray-200 dark:border-white/10 rounded-lg text-gray-400 dark:text-gray-500">
                           <p>分析或拆分结果将显示在此处</p>
                        </div>
                    )}
                </div>
