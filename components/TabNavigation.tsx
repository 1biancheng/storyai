...
const NavTab: React.FC<{ icon: React.ElementType, label: string, isActive: boolean, onClick: () => void }> = ({ icon: Icon, label, isActive, onClick }) => (
  <button
    onClick={onClick}
    className={`flex items-center gap-1 px-2 py-1 text-xs font-medium transition-colors duration-200 border-b-2 -mb-px ${
      isActive
...
const TabNavigation: React.FC<TabNavigationProps> = ({ activeView, setActiveView }) => {
  ...
  return (
    <nav className="flex items-center border-b border-gray-200 dark:border-white/10 px-2 py-0.5" role="navigation" aria-label="Main navigation">
