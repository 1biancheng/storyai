...
import * as workflowManager from '../sample_data/services/workflowManager.ts';
...
import { createProject, updateProject, deleteProject, startAutoSync } from '../sample_data/services/projectApi';
import { workspaceService } from '../sample_data/services/workspaceService.ts';
...
const StatCard: React.FC<{ label: string; value: string | number; icon: React.ElementType }> = ({ label, value, icon: Icon }) => (
    <div className="bg-white dark:bg-[#2C2C2C] p-3 rounded-lg flex items-center gap-3 border border-gray-200 dark:border-white/10">
...
className="flex-1 flex items-center justify-center gap-2 h-9 px-4 py-2 bg-primary hover:bg-primary-light text-white dark:bg-primary-light dark:hover:bg-primary font-semibold rounded-lg transition-colors text-sm"
>
...
const ProjectManager: React.FC<ProjectManagerProps> = ({ onProjectSelect }) => {
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            try {
                // 从 workspace 加载项目（不再使用 localStorage）
                const workspaceProjects = await workspaceService.getProjects();
                const projectDataList = workspaceProjects.map(metadata => 
                    workspaceService.metadataToProjectData(metadata)
                );
                ...
                console.error("从 workspace 加载数据失败", e);
            }
        };
        ...
        try {
            if (editingProjectId) {
                ...
                if (window.confirm(`您确定要保存对项目 "${originalProject?.projectName}" 的更改吗?`)) {
                    ...
                    // 更新到 workspace
                    const metadata = workspaceService.projectDataToMetadata(updatedProject);
                    await workspaceService.updateProject(editingProjectId, metadata);
                ...
                // 创建到 workspace
                const metadata = workspaceService.projectDataToMetadata(newProject);
                await workspaceService.createProject(metadata);
    ...
    const handleDeleteProject = (projectId: string) => {
        setConfirmDialog({
            visible: true,
            message: '您确定要删除此项目及其所有数据吗?此操作无法撤消.',
            onConfirm: async () => {
                try {
                    // 从 workspace 删除
    ...
    const handleSaveEditedContent = async (updatedText: string) => {
        ...
        if (updatedProject) {
            const metadata = workspaceService.projectDataToMetadata(updatedProject);
            await workspaceService.updateProject(viewingProject.id, metadata);
        }
    ...
    return (
        <div className="p-6 bg-white dark:bg-[#1E1E1E] h-full overflow-y-auto text-gray-900 dark:text-[#E2E2E2] rounded-xl shadow-md border border-gray-200 dark:border-[rgba(255,255,255,0.05)]">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="p-4 rounded-lg bg-gray-50 dark:bg-[#2C2C2C] border border-gray-200 dark:border-white/10 h-fit">
                    ...
                    <div className="space-y-2 p-3 bg-gray-100 dark:bg-black/30 border border-gray-200 dark:border-white/5 rounded-lg">
                        ...
                        <div className="flex items-center">
                            ...
                            <label htmlFor="enable-thinking-mode" className="ml-2 block text-sm">
                                启用思考模式 <span className="text-xs text-gray-500 dark:text-gray-400">(仅限 GPT-4o)</span>
                            </label>
                        </div>
                        <div className="flex items-center">
                            <input
                            ...
                            <label htmlFor="enable-search-grounding" className="ml-2 block text-sm">
                                启用搜索增强 <span className="text-xs text-gray-500 dark:text-gray-400">(仅限 GPT-4o)</span>
                            </label>
                        </div>
                    </div>
                                    ...
                                    <div key={field.id}>
                                        ...
                                        {field.type === 'boolean' ? (
                                            ...
                                            <input
                                                ...
                                                className="w-full h-8 py-1 px-2.5 border border-gray-300 dark:border-white/10 bg-white dark:bg-[#2C2C2C] text-gray-900 dark:text-white rounded-lg text-sm"
                                            />
                                        )}
                                    </div>
                                ))}
