...
import { Scene, PromptCard, FieldModel, AgentRole } from '../../types';
import { getDefaultPrompts } from './agentPrompts';
import * as agentSchemas from './agentFieldModels';
    ...
    try {
        ...
        for (const defaultModel of defaultSchemaModels) {
            if (!fieldModels.find(m => m.id === defaultModel.id)) {
                ...
                // Convert CustomSchema to Schema format expected by schemaToFieldModel
                const convertedSchema: Schema = {
                    type: 'object',
                    properties: schemaSource.properties ? Object.fromEntries(
                        Object.entries(schemaSource.properties).map(([key, prop]) => [
                            key,
                            {
                    ...
                    required: schemaSource.required
                };

                const newModel = schemaToFieldModel(convertedSchema, defaultModel.name, defaultModel.id);
