...
export const runOpenAICompatibleAgent = async (
    ...
    if (responseSchema) {
        ...
        const mid = (config.modelId || '').toLowerCase();
        if (mid.includes('gpt') || mid.includes('gemini')) {
...
export const chatCompletionsRaw = async (
    prompt: string,
    config: ModelConfig,
    extra?: { tools?: any[] }
): Promise<any> => {
    let apiUrl = config.apiUrl || 'https://api.openai.com/v1/chat/completions'
    const headers = {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${config.apiKey}`,
    }
