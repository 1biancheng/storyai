...
import { ApiResponse, ErrorResponse, ErrorCode } from '../../shared/api/contracts';
...
export class ApiClient {
  ...
  constructor(config: ApiClientConfig = {}) {
    this.baseURL = config.baseURL || (typeof window !== 'undefined' ? (window as any).VITE_BACKEND_URL : undefined) || 'http://localhost:8000';
  ...
  private normalizeApiResponse(obj: any): ApiResponse<any> {
    const ts = new Date().toISOString(); // UTC timestamp
