...
class StarPlotFormulaEngine:
    ...
    def _generate_star_elements(self, point: PlotPoint) -> Dict[str, Any]:
        ...
        return {
            "显性符号": point.star_features.get("显性符号", []),
            "隐性符号": point.star_features.get("隐性符号", []),
            "复合标签": point.star_features.get("复合标签", []),
            "evolution_triggers": point.evolution_triggers,
            "backtrack_probability": point.backtrack_probability
        }
    ...
    def _generate_star_for_element(self, element: str, point: PlotPoint) -> Dict[str, Any]:
        ...
        return {
            "显性符号": [f"{element}的显性表现"],
            "隐性符号": [f"{element}的隐性暗示"],
            "复合标签": [f"虽然{element}但是还有转折"],
    ...
    def _generate_star_transitions(self, current: PlotPoint, next_point: PlotPoint) -> Dict[str, Any]:
        ...
        return {
            "显性过渡": ["显性过渡元素1", "显性过渡元素2"],
            "隐性过渡": ["隐性过渡暗示1", "隐性过渡暗示2"],
            "复合过渡": ["虽然{current_state}但是{next_state}"],
    ...
    def analyze_story_content(self, content: str) -> Dict[str, Any]:
        ...
        analysis = {
            "显性符号": self._extract_explicit_symbols(content),
            "隐性符号": self._extract_implicit_symbols(content),
            "复合标签": self._extract_composite_labels(content),
...
if __name__ == "__main__":
    ...
    print("\n=== 内容分析结果 ===")
    print(f"显性符号数量: {len(analysis['显性符号'])}")
    print(f"隐性符号数量: {len(analysis['隐性符号'])}")
    print(f"复合标签数量: {len(analysis['复合标签'])}")
