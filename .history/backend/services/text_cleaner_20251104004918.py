...
class TextCleaner:
    ...
    def clean_to_single_line(self, text: str) -> Dict[str, Any]:
        ...
        # Step 2: Remove or replace line breaks
        line_break_count = text.count('\n') + text.count('\r')
        if line_break_count > 0:
            # Replace with spaces, preserve semantic boundaries
            text = text.replace('\r\n', ' ').replace('\r', ' ').replace('\n', ' ')
            operations.append(f"removed_{line_break_count}_line_breaks")
        
        # Step 3: Remove or replace tabs
        tab_count = text.count('\t')
        if tab_count > 0:
            text = text.replace('\t', ' ')
            operations.append(f"replaced_{tab_count}_tabs")
        
        # Step 4: Merge consecutive spaces (preserve full-width spaces)
        original_len = len(text)
        text = re.sub(r' {2,}', ' ', text)  # Only merge half-width spaces
        space_reduced = original_len - len(text)
        if space_reduced > 0:
            operations.append(f"merged_{space_reduced}_spaces")
        
        # Step 5: Remove control characters (preserve necessary spaces)
        if self.strict_mode:
        ...
        # Step 6: Remove leading and trailing whitespace
        text = text.strip()
    ...
    def validate_cleaned(self, result: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        # Check 1: No line breaks
        if '\n' in content or '\r' in content:
            errors.append("Contains line breaks")
        
        # Check 2: Byte count range (120-1800)
