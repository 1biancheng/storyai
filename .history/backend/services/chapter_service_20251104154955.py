...
from services.time_service import get_current_timestamp_ms
...
class ChapterService:
    ...
    async def create_chapter(
        ...
        try:
            ...
            if pool and asyncpg is not None:
            ...
            if pool and asyncpg is not None:
                ...
                for row in rows:
                    ...
                    if file_path.exists():
                        ...
                        chapter_data = repair_and_load(content)
                        # Only set displayOrder if it's not already present and database has a value
                        if 'displayOrder' not in chapter_data and row['display_order'] is not None:
                            chapter_data['displayOrder'] = row['display_order']
                ...
                for file in sorted(project_dir.glob("chap_*.json")):
                    ...
                    chapter_data = repair_and_load(content)
                    chapters.append(chapter_data)
                # Sort by chapterNumber for file-only mode
                chapters.sort(key=lambda ch: ch.get('chapterNumber', 0))
                return chapters
        except Exception as e:
    ...
    async def update_chapter(self, chapter_id: str, update_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            chapter['updatedAt'] = get_current_timestamp_ms()
    ...
    async def reorder_chapters(self, project_id: str, chapter_ids: List[str]) -> None:
        ...
        try:
            ...
            for idx, cid in enumerate(chapter_ids, start=1):
                ...
                ch['displayOrder'] = idx
                ch['updatedAt'] = get_current_timestamp_ms()
