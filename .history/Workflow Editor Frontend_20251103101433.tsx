                ...
                {isAgent && (
                    <>
                        ...
                        <div className="flex items-center">
                            ...
                            />
                            <label htmlFor="grounding-checkbox" className="ml-2 block text-sm font-medium text-gray-700">启用联网搜索增强</label>
                        </div>
                        ...
                        {((componentData as Tool).type === 'image_gen' || (componentData as Tool).type === 'tts') && (
                            <div>
                                ...
                                <select 
                                    ...
                                    className="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm p-2"
                                >
                                    <option value="dall-e-3">DALL-E 3 (图像)</option>
                                    <option value="tts-1">OpenAI TTS (语音)</option>
                                </select>
                            </div>
                        )}
                    </>
...
const AgentTestSection: React.FC<AgentTestSectionProps> = ({ agent }) => {
    ...
    // Mock API Call (Simulating actual OpenAI API)
    const handleTest = async () => {
        setIsLoading(true);
        setStatusMessage(`正在调用 OpenAI API (测试LLM: ${agent.llm}, 联网: ${agent.usesGrounding ? '是' : '否'})...`);
        setTestResult('');
    ...
    return (
        <div className="border-t pt-4">
            <h3 className="font-semibold text-gray-700 text-base flex items-center mb-2">
                <Send className="w-5 h-5 mr-2 text-indigo-600"/> 智能体测试 (OpenAI API)
            </h3>
            <p className="text-xs text-gray-500 mb-3">此功能用于验证提示词效果,实际调用 OpenAI API.</p>
...
const LibraryPanel: React.FC = () => {
    ...
    const addAgent = () => {
        ...
        const newAgent: Agent = { id, name: `新建智能体 (${id.split('-')[1]})`, prompt: '请在这里输入你的提示词...', llm: 'gpt-3.5-turbo', usesGrounding: false };
