...
import * as modelManager from '../services/modelManager.ts';
import * as dataService from '../services/dataService.ts'; // Import the new data service
...
const emptyFormState: Omit<ModelConfig, 'id' | 'isDefault' | 'isGemini'> = {
    name: '',
    modelId: '',
    apiKey: '',
    apiUrl: '',
...
const SettingsComponent: React.FC = () => {
    ...
    const handleSaveModel = () => {
        ...
        if (editingId) {
            ...
            const newConfig: ModelConfig = { ...formState, id: crypto.randomUUID(), isDefault: modelConfigs.length === 0, isGemini: false };
    ...
    const handleTestModel = async (config: ModelConfig) => {
        ...
        try {
            const result = config.isGemini
                ? await geminiService.testGeminiModel(config)
                    ...
                    <div className="space-y-2 mb-4">
                        {modelConfigs.map(config => (
                            <div key={config.id} className="p-3 bg-white dark:bg-[#2C2C2C] rounded-lg border border-gray-200 dark:border-white/5">
                                ...
                                {config.isGemini && (
                                    <div className="mt-2 ml-8 flex items-start gap-2 p-2 text-xs text-blue-800 bg-blue-50 border border-blue-200 dark:text-blue-300 dark:bg-blue-500/10 dark:border-blue-500/20 rounded-md">
                                       <Info size={14} className="flex-shrink-0 mt-0.5" />
                                       <span>此模型的 API 密钥通过应用程序环境配置.</span>
                                   </div>
                                )}
                            </div>
                        ))}
