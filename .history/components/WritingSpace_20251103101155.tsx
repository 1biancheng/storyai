    ...
    const handleStartProcess = async () => {
        ...
        const executeNode = async (node: WorkflowNode, context: Record<string, any>, storyContentRef: { current: StoryContent[] }, fullTextRef: { current: string }) => {
            ...
            if (activeProject.enableThinkingMode) {
                // If thinking mode is enabled, force GPT-4o and its thinking budget.
                effectiveModelId = modelManager.getModelConfigs().find(m => m.id === 'default-gpt-4o')?.id || effectiveModelId;
                if (effectiveModelId === 'default-gpt-4o') {
                    thinkingConfig = { thinkingBudget: 32768 };
                    specialFeatureLog = '已启用思考模式.强制使用 `gpt-4o` 模型并启用思考预算.';
                    modelOverrideApplied = true;
                } else {
                    specialFeatureLog = '已启用思考模式,但未找到 `gpt-4o` 配置或当前模型不是 GPT-4o.未应用思考模式.';
                }
            } else if (activeProject.enableSearchGrounding) {
                // If search grounding is enabled AND thinking mode is NOT overriding, force GPT-4o for grounding.
                effectiveModelId = modelManager.getModelConfigs().find(m => m.id === 'default-gpt-4o')?.id || effectiveModelId;
                if (effectiveModelId === 'default-gpt-4o') {
                    useSearchGrounding = true;
                    specialFeatureLog = '已启用搜索增强.强制使用 `gpt-4o` 模型并启用联网搜索.';
                    modelOverrideApplied = true;
                } else {
                    specialFeatureLog = '已启用搜索增强,但未找到 `gpt-4o` 配置或当前模型不是 GPT-4o.未应用搜索增强.';
                }
            }
