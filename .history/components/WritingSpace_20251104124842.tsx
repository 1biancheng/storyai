...
const WritingSpace: React.FC = () => {
  ...
  const startWritingProcess = async () => {
    ...
    try {
      ...
      // Initialize status
      const initialStatus: WritingProcessStatus = {
        projectId: selectedProjectId,
        workflowId: activeProject.workflowId,
      ...
      localStorage.setItem(`storyWeaverStatus_${selectedProjectId}`, JSON.stringify(initialStatus));
      ...
      if (currentStatus) {
        const failedStatus: WritingProcessStatus = {
          ...currentStatus,
          isRunning: false,
        ...
        localStorage.setItem(`storyWeaverStatus_${selectedProjectId}`, JSON.stringify(failedStatus));
      }
    } finally {
      setIsWriting(false);
    }
  ...
  const executeWorkflow = async (workflow: AgentWorkflow, project: ProjectData) => {
    ...
    for (const node of sortedNodes) {
      ...
      try {
        ...
        // Update status
        const updatedStatus: WritingProcessStatus = {
          ...currentStatus,
          currentStep: node.id,
          progress: (workflow.nodes.indexOf(node) / workflow.nodes.length) * 100
        };
    ...
    if (currentStatus && currentStatus.isRunning) {
      const completedStatus: WritingProcessStatus = {
        ...currentStatus,
      ...
      setCurrentStatus(completedStatus);
      localStorage.setItem(`storyWeaverStatus_${selectedProjectId}`, JSON.stringify(completedStatus));
  ...
  const stopWritingProcess = () => {
    ...
    const stoppedStatus: WritingProcessStatus = {
      ...currentStatus,
      isRunning: false,
      endTime: Date.now()
    };
    
    setCurrentStatus(stoppedStatus);
    localStorage.setItem(`storyWeaverStatus_${selectedProjectId}`, JSON.stringify(stoppedStatus));
