...
import { generateProjectTimeData, getCurrentTimestamp } from '../utils/timeUtils.ts';
...
const WritingSpace: React.FC = () => {
  ...
  // Computed isWriting based on currentStatus
  const isWriting = currentStatus !== null && 
                    currentStatus !== WritingProcessStatus.IDLE && 
                    currentStatus !== WritingProcessStatus.COMPLETE && 
                    currentStatus !== WritingProcessStatus.ERROR;
  ...
  const handleCreateProject = () => {
    ...
    const newProject: ProjectData = {
      ...
      ...generateProjectTimeData(), // 使用统一的时间函数
    };
  ...
  const executeWorkflow = async (workflow: AgentWorkflow, project: ProjectData) => {
    ...
    for (const node of sortedNodes) {
      if (!currentStatus || currentStatus === WritingProcessStatus.IDLE) break;
    ...
    // Complete workflow
    if (currentStatus && currentStatus !== WritingProcessStatus.IDLE) {
  ...
  const executeAgentNode = async (node: WorkflowNode, project: ProjectData) => {
    ...
    if (projectData.nodeResults && Object.keys(projectData.nodeResults).length > 0) {
      prompt += '\n\n已完成步骤:';
      for (const nodeId of Object.keys(projectData.nodeResults)) {
    ...
    const result = {
      ...
      timestamp: getCurrentTimestamp() // 使用统一的时间函数
    };
  ...
  const getStatusIcon = () => {
    ...
    switch (currentStatus) {
      case WritingProcessStatus.ERROR:
        return <AlertCircle className="w-5 h-5 text-red-500" />;
      case WritingProcessStatus.COMPLETE:
        return <CheckCircle className="w-5 h-5 text-green-500" />;
      case WritingProcessStatus.IDLE:
  ...
  const getStatusText = () => {
    ...
    return currentStatus; // WritingProcessStatus枚举值已经是中文描述
  };
