        ...
        <div className="w-80 border-l border-gray-200 bg-white overflow-hidden flex flex-col">
          ...
          {showPromptCards && (
            <div className="fixed inset-0 z-50 flex">
              ...
              <div 
                className="absolute inset-0 bg-black bg-opacity-20"
                onClick={() => setShowPromptCards(false)}
              />
              {/* Panel content */}
              <div className="absolute top-0 right-0 bottom-0 w-80 bg-white border-l border-gray-200 shadow-xl flex flex-col">
          ...
          {showFileUpload && (
            <div className="fixed inset-0 z-50 flex">
              ...
              <div className="absolute top-0 right-0 bottom-0 w-80 bg-white border-l border-gray-200 shadow-xl flex flex-col">
                <div className="px-4 py-3 flex justify-between items-center border-b border-gray-200">
                  <h3 className="font-medium text-gray-900">文件附件</h3>
                  <button
                    onClick={() => setShowFileUpload(false)}
                    className="p-1 rounded-md text-gray-400 hover:text-gray-600"
                  >
                ...
                <div className="flex-1 p-4 overflow-y-auto">
                  <div className="mb-4">
                    ...
                    <label
                      htmlFor="file-upload"
                      className="block w-full px-4 py-2 text-center border border-dashed border-gray-300 rounded-md cursor-pointer hover:bg-gray-50 transition-colors"
                    >
                      <Upload size={18} className="mx-auto mb-1 text-gray-400" />
                      <span className="text-sm text-gray-600">点击或拖拽上传文件</span>
                    </label>
                  </div>
                  
                  {fileAttachments.length === 0 ? (
                    <p className="text-sm text-gray-500">暂无文件附件</p>
                  ) : (
                    <div className="space-y-2">
                      {fileAttachments.map(file => (
                        ...
                        >
                          ...
                          <button
                            onClick={() => handleDeleteFileAttachment(file.id)}
          ...
          {showContentLinks && (
            <div className="fixed inset-0 z-50 flex">
              ...
              <div 
                className="absolute inset-0 bg-black bg-opacity-20"
                onClick={() => setShowContentLinks(false)}
              />
              {/* Panel content */}
              <div className="absolute top-0 right-0 bottom-0 w-80 bg-white border-l border-gray-200 shadow-xl flex flex-col">
                <div className="px-4 py-3 flex justify-between items-center border-b border-gray-200">
                  <h3 className="font-medium text-gray-900">内容链接</h3>
                  <button
                    onClick={() => setShowContentLinks(false)}
                    className="p-1 rounded-md text-gray-400 hover:text-gray-600"
                  >
                ...
                <div className="flex-1 p-4 overflow-y-auto">
                    ...
                    <button
                      onClick={() => {
                        if (selectedChapterId) {
                          ...
                          if (title) {
                            handleCreateContentLink(title, 'chapter', selectedChapterId);
                          }
                        } else {
                          alert('请先选择一个章节');
                        }
                  ...
                  {contentLinks.length === 0 ? (
                    <p className="text-sm text-gray-500">暂无内容链接</p>
                  ) : (
                    <div className="space-y-2">
