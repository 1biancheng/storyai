...
// Define custom types to replace Google GenAI types
enum CustomType {
  OBJECT = 'object',
  ARRAY = 'array',
  STRING = 'string',
...
export const chapterWriterSchema: Schema = {
    type: Type.OBJECT,
    properties: {
        chapter_title: { type: Type.STRING, description: "The full title of the chapter being written." },
        chapter_content: { type: Type.STRING, description: "The full text content of the chapter." },
        new_characters_introduced: {
            ...
            items: {
                type: Type.OBJECT,
                properties: {
                    name: { type: Type.STRING, description: "Name of the new character." },
                    brief_description: { type: Type.STRING, description: "A brief description of their appearance, role, and how they were introduced." },
                },
        ...
        setting_description: { type: Type.STRING, description: "A summary of the primary setting and atmosphere of this chapter." }
    },
    required: ["chapter_title", "chapter_content"]
};

// New Schema for the Section Writer Agent
export const sectionWriterSchema: Schema = {
    type: Type.OBJECT,
    properties: {
        original_section: { type: Type.STRING, description: "The original text that was provided for revision." },
        analysis: { type: Type.STRING, description: "A brief analysis of what was lacking in the original text (e.g., 'lacked sensory details', 'passive voice')." },
        revised_section: { type: Type.STRING, description: "The new, improved text for the section." },
        key_improvements: {
            type: Type.ARRAY,
            description: "A list of specific improvements that were made.",
            items: { type: Type.STRING, description: "e.g., 'Added olfactory details', 'Strengthened verb choices', 'Improved pacing'." }
        }
    },
...
const reviewIssueSchema: Schema = {
    type: Type.OBJECT,
    properties: {
        issue_description: { type: Type.STRING, description: "A clear and concise description of the issue found." },
        suggestion: { type: Type.STRING, description: "A specific, actionable suggestion for how to fix the issue." },
        location_reference: { type: Type.STRING, description: "An optional reference to where the issue occurs, e.g., 'In Chapter 3, paragraph 2'." }
    },
...
export const reviewerSchema: Schema = {
    ...
    properties: {
        word_count_analysis: {
            type: Type.OBJECT,
            properties: {
                current_word_count: { type: Type.INTEGER },
                target_word_count: { type: Type.INTEGER },
                status: { type: Type.STRING, description: "Either '达标' or '未达标'." }
            },
        ...
        content_quality_review: {
            type: Type.OBJECT,
            properties: {
                plot_and_logic: { type: Type.ARRAY, description: "A list of plot or logic issues.", items: reviewIssueSchema },
                pacing_and_structure: { type: Type.ARRAY, description: "A list of pacing or structural issues.", items: reviewIssueSchema },
        ...
        overall_summary: { type: Type.STRING, description: "A high-level summary of the review findings." }
    },
...
export const editorSchema: Schema = {
    type: Type.OBJECT,
    properties: {
        final_manuscript: { type: Type.STRING, description: "The complete, final, edited text of the novel." },
        edit_summary_log: {
            type: Type.ARRAY,
            ...
            items: {
                type: Type.OBJECT,
                properties: {
                    change_description: { type: Type.STRING, description: "e.g., 'Restructured Chapter 5 for better pacing', 'Rewrote dialogue in the climax'." },
                    reasoning: { type: Type.STRING, description: "Why the change was made, referencing the reviewer or quality evaluator's feedback." }
                },
...
export const qualityEvaluatorSchema: Schema = {
    type: Type.OBJECT,
    properties: {
        overall_score: { type: Type.NUMBER, description: "The final score from 1 to 10." },
        dimensional_scores: {
            type: Type.ARRAY,
            ...
            items: {
                type: Type.OBJECT,
                properties: {
                    dimension: { type: Type.STRING, description: "The dimension being scored, e.g., '故事性', '文笔', '创新性', '角色深度'." },
                    score: { type: Type.NUMBER, description: "The score for this dimension (1-10)." },
                    justification: { type: Type.STRING, description: "The reason for the given score." },
                    suggestion: { type: Type.STRING, description: "Specific advice for how to improve in this dimension." }
                },
                required: ["dimension", "score", "justification", "suggestion"]
            }
        },
        final_verdict: { type: Type.STRING, description: "A summary paragraph of the evaluation and overall thoughts." }
    },
    required: ["overall_score", "dimensional_scores", "final_verdict"]
};

// New Schema for the Summary Agent
export const summaryAgentSchema: Schema = {
    type: Type.OBJECT,
    properties: {
        core_plot_summary: { type: Type.STRING, description: "A concise summary of the main events of the chapter." },
        continuity_points: {
            type: Type.OBJECT,
            properties: {
                characters_present: { type: Type.ARRAY, description: "List of characters who appeared or were significantly mentioned.", items: { type: Type.STRING } },
                key_locations: { type: Type.ARRAY, description: "List of settings where the chapter took place.", items: { type: Type.STRING } },
                foreshadowing_elements: { type: Type.ARRAY, description: "List of hooks, clues, or foreshadowing elements that were planted.", items: { type: Type.STRING } },
                resolved_threads: { type: Type.ARRAY, description: "List of previous plot points or questions that were resolved.", items: { type: Type.STRING } }
            },
            required: ["characters_present", "key_locations", "foreshadowing_elements", "resolved_threads"]
        },
        character_state_update: { type: Type.STRING, description: "A description of how the main characters' emotional states, goals, or relationships have changed by the end of the chapter." },
        next_chapter_directive: { type: Type.STRING, description: "The single most important instruction for the writer of the next chapter to ensure continuity." }
    },
...
const agentSchemaMap: Partial<Record<AgentRole, Schema>> = {
 ...
 */
export const getSchemaForAgent = (role: AgentRole): Schema | null => {
 ...
 */
export const validateData = (data: any, schema: Schema): { isValid: boolean, missingFields: string[] } => {
   ...
   */
  function check(obj: any, schemaNode: Schema, path: string = '') {
    // Only validate objects with properties.
    if (schemaNode.type !== Type.OBJECT || !schemaNode.properties || typeof obj !== 'object' || obj === null) {
      return;
    }
    ...
    for (const key of required) {
      ...
      if (propertySchema.type === Type.OBJECT) {
        check(value, propertySchema, fullPath);
      
      // Handle arrays: check items if the array is not empty.
      } else if (propertySchema.type === Type.ARRAY) {
        if (!Array.isArray(value)) {
            ...
            if (itemSchema && value.length > 0) {
                for (let i = 0; i < value.length; i++) {
                    check(value[i], itemSchema, `${fullPath}[${i}]`);
                }
            }
        }
      }
