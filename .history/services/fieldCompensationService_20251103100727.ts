...
import { AgentRole, PromptCard } from '../types.ts';
import * as agentService from './agentService.ts';
import * as agentFieldModels from './agentFieldModels.ts';
...
const normalizeToSchema = (data: any, schema: Schema): any => {
...
const buildDefaultObject = (schemaNode: Schema): any => {
    const obj: any = {};
...
const defaultValueForSchema = (schemaNode: Schema): any => {
...
const ensureRequiredFields = (data: any, schemaNode: Schema): any => {
...
export const validateAndCompensate = async (
    ...
    while (attempts < MAX_RETRIES && !isValid) {
        ...
        if (attempts > 1) {
            ...
            const response = await agentService.runAgent(promptWithCompensation, schema);
