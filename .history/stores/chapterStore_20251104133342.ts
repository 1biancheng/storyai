...
export const useChapterStore = create<ChapterStore>((set, get) => {
  ...
  saveChapter: async (chapterId, updates) => {
    ...
    try {
      ...
      set((state) => ({
        chapters: state.chapters.map((ch) =>
          ch.id === chapterId ? { ...ch, ...updatedChapter, updatedAt: Date.now() } : ch
        )
      }));
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unknown error';
  ...
  reorderChaptersAPI: async (projectId: string, chapterIds: string[]) => {
    set({ isLoading: true, error: null });
    try {
      // Send chapter IDs directly in the request body to match backend's expected format
      await apiClient.post<void>(`${API_BASE}/project/${projectId}/reorder`, chapterIds);
