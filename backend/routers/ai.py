...
from api_framework import ApiException  # {{ line 15-15 logic+clean fix | 来源: ensure ApiException is imported for proper exception mapping }}
...
_rate_counters: Dict[str, list] = {}
def _allow_request(key: str, limit: int, window_sec: int) -> bool:
    import time
    now = time.time()
...
async def run_agent_stream(
    ...
    forceTools: Optional[str] = None,  # 逗号分隔的强制工具列表
):
    ...
    try:
        ...
        # 强制工具系统提示注入
        if forceTools:
            tools = [t.strip() for t in forceTools.split(",") if t.strip()]
            if tools:
                sys_prompt = f"【系统】用户已强制启用以下工具,你必须在本次回复中主动调用它们:{', '.join(tools)}."
                prompt = f"{sys_prompt}\n\n{prompt}"
        ...
        async def generate_stream():
            try:
                ...
                ok = _allow_request(f"{resolved_model_id}", 20, 60)
                if not ok:
                    yield sse_error_event(DomainError.SSE_STREAM_ERROR, "rate limited")
                    return
...
async def get_model_config(model_id: str):
    ...
    try:
        ...
        # {{ line ~245-258 logic+clean fix | 来源: correct ApiException handling to return 404 for unsupported/not found }}
        detail_text = str(getattr(e, "detail", e))
        # Map unsupported or not found to 404
        if e.status_code == 404 or "unsupported" in detail_text.lower() or "not found" in detail_text.lower():
            raise HTTPException(status_code=404, detail=f"Model {model_id} not found")
        # For other ApiException, propagate original status code
        logger.error(f"Error getting model config: {detail_text}")
        raise HTTPException(status_code=getattr(e, "status_code", 500), detail=detail_text)
    except Exception as e:
        logger.error(f"Error getting model config: {str(e)}")
        # 对于不支持的模型,返回404而不是500
...
async def test_model(request: ModelTestRequest):
    ...
    try:
        ...
        result = await ai_service.test_model_connection(
            model_id=request.model_id
            # 移除了test_prompt参数,因为方法定义中没有这个参数
        )
