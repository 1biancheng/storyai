...
async def execute_agent_node(
    ...
    try:
        ...
        result = await ai_service.run_agent(
            prompt=final_prompt,
            model_id=agent_config.model_id or "gpt-3.5-turbo",
        ...
        return {
            ...
            "output": result,
            "model_used": agent_config.model_id or "gpt-3.5-turbo",
            "timestamp": datetime.now().isoformat()
        }
    except Exception as e:
...
async def execute_tool_node(
    ...
    try:
        if tool_config.tool_type == "http_request":
        ...
        elif tool_config.tool_type == "web_search":
            # 导入网络搜索服务
            from routers.tools import WebSearchService, SearchEngineConfig
            
            # 获取搜索参数
            query = tool_params.get("query", "")
            num_results = tool_params.get("num_results", 10)
            
            if not query:
                raise ValueError("网络搜索需要提供查询关键词")
            ...
            if engine == "google":
                if not config.get("api_key") or not config.get("search_engine_id"):
                    raise ValueError("Google搜索未配置: 需要设置google_search_api_key和google_search_engine_id")
                ...
                if not config.get("api_key"):
                    raise ValueError("Bing搜索未配置: 需要设置bing_search_api_key")
