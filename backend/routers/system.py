...
async def get_server_time(request: Request):
    ...
    This endpoint is used by the frontend to display synchronized time.
    ...
    try:
        ...
        response = success_response(time_data, "Server time retrieved successfully", get_request_id(request))
        log_response(request, response)
        return response
